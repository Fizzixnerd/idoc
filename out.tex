\documentclass[letterpaper]{memoir}\usepackage{hyperref}\usepackage{graphicx}\usepackage{minted}\usepackage[usenames,dvipsnames]{xcolor}\usepackage{amsmath}\usepackage{amsthm}\usepackage{amssymb}\usepackage{mdframed}\newtheorem{Theorem}{Theorem}\newtheorem{Lemma}[Theorem]{Lemma}\newtheorem{Corollary}[Theorem]{Corollary}\newtheorem{Proposition}[Theorem]{Proposition}\newtheorem{Conjecture}[Theorem]{Conjecture}\newtheorem{Axiom}[Theorem]{Axiom}\theoremstyle{definition}\newtheorem{Definition}{Definition}\newmdenv[]{SideNote}\newmdenv[backgroundcolor=SkyBlue]{Info}\newmdenv[backgroundcolor=Goldenrod]{Caution}\newmdenv[backgroundcolor=BrickRed]{Warning}\setcounter{tocdepth}{2}\begin{document}\title{idoc User Guide}\author{ILS Contributors}\date{\today{}}\maketitle{}\tableofcontents{}\chapter{\label{idocUserGuide}idoc User Guide}\subsubsection{Prerex}\href{}{http://www.independentlearning.science/tiki/None}: Someone wanted to see what this looked like, so it's here.
...
\section{Goals}\begin{Info}[frametitle=\label{sourceTip}Get the Source!]You will probably want to examine the source for this document along
...
\href{httpswww.haskell.org}{Haskell programming language} to make the
...
\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
...
\href[secLinks]{section on links}.
...
\begin{enumerate}\item This is an unordered list.  It can contain paragraph contents.\item It will look like bullet points in the final render.\item Third main item.\end{enumerate}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
...
\begin{enumerate}\item This is a numbered list.\item This is the second guy.\item And so on...\end{enumerate}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
...
\begin{enumerate}\item[\textbf{First Item}]This is a list where the items have labels.\item[\textbf{Second Item}]Another item.\item[\textbf{And So On}]...\end{enumerate}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
...
\label{eqnFDefn}\[f(x) = \int_x^\infty g(t) \mathrm{d}t\]\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
 @math
...
g(y) &=& y^2
\implies (f \circ g) (x) &=& 2 x^2 + 3\end{align}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
 @eqnarray
 ---
 f(x) &=& 2x + 3
 g(y) &=& y^2
...
Then we can use \texttt{ref\{\}} or \texttt{\href[idLabel]{}} syntax to refer to them:

\begin{align}\label{importantEquation} y &=& mx + b\end{align}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
 @eqnarray
 ---
...
to \href[importantEquation]{the important equation}.  If you want to
prevent an equation from being numbered, use \texttt{nonumber} as usual.

\begin{align}R_{\mu \nu} - \frac{1}{2} R g_{\mu \nu} + \Lambda g_{\mu \nu} &=& \frac{8 \pi G}{c^4} T_{\mu \nu} \nonumber\end{align}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
 @eqnarray
 ---
 R_{\mu \nu} - \frac{1}{2} R g_{\mu \nu} + \Lambda g_{\mu \nu} &=& \frac{8 \pi G}{c^4} T_{\mu \nu} \nonumber
 ---
...
\begin{align}x \cdot y = y \cdot x\end{align}\end{Axiom}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
 @axiom
 ---
...
\end{Definition}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
 [term=number]
 @definition
 ---
...
\end{proof}\end{Theorem}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
 @theorem
 ---
...
\end{proof}\end{Lemma}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
 @lemma
 #Easy Lemma
 ---
...
\end{Proposition}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
 @proposition
 ---
...
\end{Corollary}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
 @corollary
 ---
...
\end{Conjecture}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
 @conjecture
 ---
...
\end{proof}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
...
\label{qEinstein}\begin{quote}The internet is the most important invention since gravity.\end{quote}\begin{minted}[frame=single,framesep=5mm,breaklines]{text}
...
\begin{Info}[frametitle=\label{tipBlock}This is a Tip Block]This will show a little lightbulb symbol next to it.
...
\subsubsection{But What \emph{is} the Determinant?! -- An Example Intuition Block}The determinant from a geometric point of view is used to talk about
...
\texttt{\href{httpwww.example.com}{this}} and appears like
\href{httpwww.example.com}{this}.  Note that the \texttt{http(s)://} part of
...
\texttt{\href[important-equation]{this}}, and appear like
\href[important-equation]{this}.  That is, they are the \texttt{\#} symbol
...
\texttt{\{\}}.  This will just use the text inside the \texttt{\href[WTF]{}} as the display
...
\href{httpwww.independentlearning.science/uusBbd5.jpg}{}Bunnies make everything better.
