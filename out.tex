\documentclass[letterpaper]{memoir}\usepackage{hyperref}\usepackage{graphicx}\usepackage{amsmath}\usepackage{amsthm}\usepackage{amssymb}\begin{document}\title{Title}\author{ILS Contributors}\date{\today{}}\maketitle{}\tableofcontents{}\chapter{\label{idocUserGuide}idoc User Guide}\subsubsection{Prerex}\href{}{http://www.independentlearning.science/tiki/None}: Someone wanted to see what this looked like, so it's here.
      Normally you can just leave the prerex block out when
      there are none.\subsection{Introduction}idoc is a language and a program.  The language is what was used to
...
\section{Goals}\subsubsection{\label{sourceTip}Get the Source!}You will probably want to examine the source for this document along
...
\begin{enumerate}\item[Prerequisite resolution]This is the big one.  The website is
...
have been \emph{impossible} to add this to Markdown.\item[Typed Blocks]A "strongly-typed" block is where one declares the
...
care of that for you.\item[Extensible Blocks]On the other hand, having "extensible" blocks
...
\emph{easy} in idoc.\item[Easy to Learn by Example]While Asciidoc is a powerful markup
...
allows them to do this.\item[Power with Limitations]Asciidoc and Markdown allow you to insert
...
instead of having them rely on such "escape hatches".\item[Haskell Implementation]We are using the
\ref{Haskell programming language} to make the
...
the document and looks like this: \texttt{= Title}.  The space between the
\texttt{=} and the title text is important.  To make new \textbf{sections}, type \texttt{==
Section Name}.  Note that the section title must be separated from the
previous content by a newline.  For example,

\verb`I am a paragraph!  Hear me roar!

==New Section!

Another paragraph.  RAAAAAAAAAAAWR!`is correct, while

\verb`I am a paragraph!  Hear me roar!
==New Section!

Another paragraph.  RAAAAAAAAAAAWR!`would not display what you want correctly.  Subsections are the same
...
\begin{enumerate}\item This is an unordered list.  It can contain paragraph contents.\item It will look like bullet points in the final render.\item Third main item.\end{enumerate}\verb`-This is an unordered list.  It can contain paragraph contents.

-It will look like bullet points in the final render.

-Third main item.`\begin{enumerate}\item This is a numbered list.\item This is the second guy.\item And so on...\end{enumerate}\verb`.This is a numbered list.

.This is the second guy.

.And so on...`\begin{enumerate}\item[First Item]This is a list where the items have labels.\item[Second Item]Another item.\item[And So On]...\end{enumerate}\verb`:First Item::This is a list where the items have labels.

:Second Item::Another item.

:And So On::...`\section{\label{blocks}Blocks}\textbf{Blocks} are a fundamental construct in idoc.  They all look the same,
...
\label{eqnFDefn}\[f(x) = \int_x^\infty g(t) \mathrm{d}t\]\verb` @math
 #Look Ma', I Have Equations!
 ---
 f(x) = \int_x^\infty g(t) \mathrm{d}t
 ---
 [[#eqnFDefn]]{Basic Math}`Note that that $d$ will not be upright as it should be.  We'll fix
...
There is also a more specialized \texttt{eqnarray} block, which numbers
equations.  Note however that \emph{unlike} the $\LaTeX$ eqnarray, you do
not put a linebreak (\texttt{\textbackslash{} }) at the end of each line.  The ampersands
(\texttt{\&}) tell MathJax where to align line to.  So, for example, the
equations below have a "left side", an "equals sign" in the middle,
and a "right side".

\begin{align}f(x) &=& 2x + 3
g(y) &=& y^2
\implies (f \circ g) (x) &=& 2 x^2 + 3\end{align}\verb` @eqnarray
 ---
 f(x) &=& 2x + 3
 g(y) &=& y^2
 \implies (f \circ g) (x) &=& 2 x^2 + 3
 ---`Note that we can use the standard \texttt{label\{\}} macro within the \texttt{math}
or \texttt{eqnarray} environments to assign IDs to individual equations.
Then we can use \texttt{ref\{\}} or \texttt{\ref{}} syntax to refer to them:

\begin{align}\label{importantEquation} y &=& mx + b\end{align}\verb` @eqnarray
 ---
 \label{importantEquation} y &=& mx + b
 ---`Refer to the \emph{important equation} $\ref{importantEquation}$.  Refer
to \ref{the important equation}.  If you want to
prevent an equation from being numbered, use \texttt{nonumber} as usual.

\begin{align}R_{\mu \nu} - \frac{1}{2} R g_{\mu \nu} + \Lambda g_{\mu \nu} &=& \frac{8 \pi G}{c^4} T_{\mu \nu} \nonumber\end{align}\verb` @eqnarray
 ---
 R_{\mu \nu} - \frac{1}{2} R g_{\mu \nu} + \Lambda g_{\mu \nu} &=& \frac{8 \pi G}{c^4} T_{\mu \nu} \nonumber
 ---`\begin{Axiom}An axiom looks like this.

\begin{align}x \cdot y = y \cdot x\end{align}\end{Axiom}\verb` @axiom
 ---
...
\end{Definition}\verb` [term=number]
 @definition
 ---
 a *number* is an element of an field.
 ---
 [[#def-number]]{Definition: Number}`\label{thm-foil}\begin{Theorem}\begin{align}(x + y)^2 = x^2 + 2 xy + y^2.\end{align}\begin{proof}Use geometry.

\end{proof}\end{Theorem}\verb` @theorem
 ---
 ...
 Use geometry.
 ---
 [[#thm-foil]]{Theorem: FOIL}`\label{lemEasyLemma}\begin{Lemma}This is a lemma.  It's pretty easy to prove.

For $n > 2$ there exists no integer solutions to the equation:

\begin{align}x^n + y^n = z^n\end{align}\begin{proof}[Easy Lemma]It's trivial.

\end{proof}\end{Lemma}\verb` @lemma
 #Easy Lemma
 ---
...
\label{qEinstein}\begin{quote}The internet is the most important invention since gravity.\end{quote}\verb` [author=Albert Einstein]
 @quote
 #A Quote from the "Great One".
 ---
 The internet is the most important invention since gravity.
 ---
 [[#qEinstein]]{Einstein Quote}`Notice in the \texttt{quote} block we added an author attribution above as an
...
\subsubsection{Please be Aware:}This content will be rendered off to the side of the document.  You are
...
\subsubsection{\label{warningBlock}This is a Warning Block}This will show a little warning symbol next to it.
...
\verb` @warning
 ...
 ---
 [[#warningBlock]]{Warning}`\subsubsection{\label{infoBlock}This is an Info Block}This will show a little info symbol next to it.
...
\verb` @info
 ...
 ---
 [[#infoBlock]]{Info}`\subsubsection{\label{cautionBlock}This is a Caution Block}This will show a little caution symbol next to it.
...
\verb` @caution
 ...
 ---
 [[#cautionBlock]]{Caution}`\subsubsection{\label{tipBlock}This is a Tip Block}This will show a little lightbulb symbol next to it.
...
\verb` @tip
 ...
 ---
 [[#tipBlock]]{Tip}`If you're worried about all the wasted space in the HTML output, don't
...
\subsection{\label{myConnection}A connection}\subsubsection{Connection Prerex}\href{}{http://www.independentlearning.science/tiki/Basics/Logic}: Mostly only \emph{first-order} logic will be used, but some \emph{second-order} stuff is important for stuff later on, so we put it here.\href{}{http://www.independentlearning.science/tiki/Basics/Fun}: And so on...A \texttt{connection} is useful when you have content connected to the main text
...
\subsubsection{\label{renderingInfo}Rendering}If you're viewing this full-screen on a computer, you'll notice that
...
\ref{on topology}, we might want to talk
...
\verb" @connection
 ...
 depending on if the reader has the necessary prereqs or not.
 ---
 [[#myConnection]]{A Connection}"\subsubsection{But What \emph{is} the Determinant?! -- An Example Intuition Block}The determinant from a geometric point of view is used to talk about
...
\verb` @example
 ...
 First, assume not.  But that's not the case.  QED.
 ---
 [[#exExample]]{An Example}`\subsubsection{Exercise}What is largest rational number that can be written as a fraction of
$5$ and $7$?
...
\texttt{\ref{this}} and appears like
\ref{this}.  Note that the \texttt{http(s)://} part of
...
\texttt{\ref{this}}, and appear like
\ref{this}.  That is, they are the \texttt{\#} symbol
...
\ref{this right here}.  However, the \texttt{\#}
...
\texttt{\{\}}.  This will just use the text inside the \texttt{\ref{}} as the display
...
\label{ytUnlimitedCorners}\verb`THIS ISN'T IMPLEMENTED (YouTube): `\subsection{Images}We can embed important images too.

\verb`THIS ISN'T IMPLEMENTED (Image): `Bunnies make everything better.
